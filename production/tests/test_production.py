...
GENERATED_LEDGER = ROOT / 'production/runtime/persistent_ledger.py'
GENERATED_API = ROOT / 'production/runtime/platform_api.py'
...
class RecordingActuator:
    """Explicit test double kept in tests, never generated into production runtime."""
    def __init__(self):
        self.artifacts = []

    def actuate(self, artifact):
        self.artifacts.append(artifact)
        return b'test-consequence:' + artifact.payload, True
...
class ProductionClosureTests(unittest.TestCase):
    ...
    @unittest.skipUnless(GENERATED_PLATFORM.is_file(), 'generated consequences unavailable in source-only checkout')
    def test_production_runtime_has_no_in_memory_actuator(self):
        text = GENERATED_PLATFORM.read_text(encoding='utf-8')
        self.assertIn('class ProductionActuator', text)
        self.assertNotIn('class MemoryActuator', text)

    @unittest.skipUnless(GENERATED_LEDGER.is_file() and GENERATED_DFCM.is_file(), 'generated consequences unavailable in source-only checkout')
    def test_persistent_receipt_ledger_replays(self):
        from production.runtime import dfcm_generated as dfcm
        from production.runtime.persistent_ledger import SqliteReceiptLedger
        with tempfile.TemporaryDirectory() as tmp:
            ledger = SqliteReceiptLedger(str(Path(tmp) / 'receipts.sqlite3'))
            actuator = RecordingActuator()
            from production.runtime import generated_platform as platform
            broker = platform.PlatformBroker(actuator, source_identity='test-source', base_identity='test-base', environment_identity='test-env', authority='test-authority', ledger=ledger)
            broker.execute('software-delivery', 'durable-1', 'service/orders', b'release-v2')
            reopened = SqliteReceiptLedger(str(Path(tmp) / 'receipts.sqlite3'))
            self.assertEqual(1, len(reopened.receipts))
            self.assertTrue(reopened.replay())
            self.assertNotEqual('GENESIS', reopened.head)

    @unittest.skipUnless(GENERATED_API.is_file(), 'generated consequences unavailable in source-only checkout')
    def test_generated_api_is_real_broker_surface(self):
        text = GENERATED_API.read_text(encoding='utf-8')
        for marker in ('ThreadingHTTPServer', '/v1/intents', 'SqliteReceiptLedger', 'ProductionActuator', 'PLATFORM_API_TOKEN'):
